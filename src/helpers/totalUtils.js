...
// MUDANÇA: Adicionado parâmetro 'source' para filtrar por plataforma.
export async function calculateTotalIncome(userId, month = null, source = null) {
...
// MUDANÇA: Simplificado para usar igualdade em vez de regex para categorias 'enum'.
...
// NOVO: Função central para atender à intenção "get_summary".
export async function getPeriodSummary(userId, month, monthName, source, category) {
  if (source) {
    const total = await calculateTotalIncome(userId, month, source);
    return `💰 Ganhos com *${source}* em _${monthName}_: *R$ ${total.toFixed(2)}*`;
  }
  if (category) {
    const total = await calculateTotalExpenses(userId, category, month);
    return `💸 Gastos com *${category}* em _${monthName}_: *R$ ${total.toFixed(2)}*`;
  }
...
// NOVO: Prepara os dados para o gráfico de lucratividade.
export async function getProfitReportData(userId, days) {
  ...
  const [incomes, expenses] = await Promise.all([incomePromise, expensePromise]);
  
  // Combina os dados em um único formato para o script Python
  const dataMap = {};
  incomes.forEach(item => {
      dataMap[item._id] = { ...dataMap[item._id], date: item._id, income: item.total };
  });
  expenses.forEach(item => {
      dataMap[item._id] = { ...dataMap[item._id], date: item._id, expense: item.total };
  });
...
// MUDANÇA: Exibe os ganhos agrupados por plataforma.
export async function getIncomeDetails(userId, month, monthName) {
    ...
    incomes.forEach(income => {
      ...
      incomesBySource[source].push(`   💰 ${income.description}: *R$ ${income.amount.toFixed(2)}*`);
    });
...
// MUDANÇA: Lógica mantida, mas ajustada para clareza.
export async function getExpenseDetails(userId, month, monthName) {
    ...
    const expenses = await Expense.find(matchStage).sort({ category: 1, date: 1 });
...
// MUDANÇA: Atualizado para usar `reminderDate` e exibir o `type` do lembrete.
export async function getTotalReminders(userId) {
  ...
  const typeEmoji = { 'Pagamento': '💳', 'Manutenção': '🔧', 'Documento': '📄', 'Outro': '🗓️' };

  return reminders.map(r => {
      const dateObj = new Date(r.reminderDate);
      const formattedDate = dateObj.toLocaleDateString("pt-BR", { timeZone: 'UTC' });
      return `${typeEmoji[r.type] || '🗓️'} *${r.type}:* ${r.description} - *${formattedDate}* (#${r.messageId})`;
    }).join("\n");
}